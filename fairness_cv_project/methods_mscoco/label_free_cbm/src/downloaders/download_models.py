...
def download_all_models(root: Path = Path.cwd()):
    ...
    folder_save = root / "saved_models" / "clip_models"
...
def _download_and_save(url, save_path):
    ...
    if os.path.isfile(save_path):
        if hashlib.sha256(open(save_path, "rb").read()).hexdigest() == expected_sha256:
            ...
            warnings.warn(
                f"{save_path} exists, but the SHA256 checksum does not match; re-downloading the file"
            )
...
def download_sentence_transformer(
    save_path=Path.cwd() / "saved_models" / "sentence_transformer.pt",
):
