...
from fairness_cv_project.methods_mscoco.label_free_cbm.src.utils import (
    data_utils,
    utils,
)
import fairness_cv_project.methods_mscoco.label_free_cbm.src.similarity as similarity
from fairness_cv_project.methods_mscoco.label_free_cbm.src.glm_saga.elasticnet import (
...
def train_cbm_and_save(args):
    ...
    if args.concept_set == None:
        args.concept_set = "data/concept_sets/{}_gpt3_ensemble2.txt".format(
            args.dataset
        )
    ...
    concepts_idx = [
        ...
        if highest[i] > args.clip_cutoff or concepts[i] in protected_concepts
    ]
    concepts = [
        concepts[i]
        for i in range(len(concepts))
    ...
    if args.print:
        for i, concept in enumerate(concepts):
            if (
                sim[i] <= args.interpretability_cutoff
                and concept not in protected_concepts
            ):
    ...
    concepts_idx = [
        i
        for i in range(len(concepts))
        if interpretable[i] or concepts[i] in protected_concepts
    ]
    concepts = [
        concepts[i]
        for i in range(len(concepts))
    ...
    logging.info(
        "Creating dataset with interpretable concepts predicted by the projection layer and final targets"
    )
    ...
    logging.info(
        "Training the final linear model from interpretable concepts to final targets"
    )
    STEP_SIZE = 0.1
    ALPHA = 0.99
    metadata = {}
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="mscoco_single")
    parser.add_argument(
    ...
    parser.add_argument(
        "--clip_name", type=str, default="ViT-B/16", help="Which CLIP model to use"
    )
    ...
    parser.add_argument(
        "--name_suffix", type=str, default="", help="Suffix to add to saved model name"
    )
    parser.add_argument(
        "--protected_concepts", nargs="+", default=[], help="protected concepts"
    )
