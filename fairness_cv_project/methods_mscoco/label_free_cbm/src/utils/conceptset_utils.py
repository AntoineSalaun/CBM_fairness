...
MODEL_PATH = "/home/gridsan/hzli/CV-Fairness/saved_models/all-mpnet-base-v2"
...
def get_init_conceptnet(
    ...
    for cls in tqdm(classes):
        ...
        for word in words:
            obj = requests.get(
                "http://api.conceptnet.io/c/en/{}?limit={}".format(word, limit)
            ).json()
            obj.keys()
            for dicti in obj["edges"]:
                ...
                try:
                    if (
                        dicti["start"]["language"] != "en"
                        or dicti["end"]["language"] != "en"
                    ):
                        continue
                except KeyError:
                    continue
...
def _clip_dot_prods(list1, list2, device="cuda", clip_name="ViT-B/16", batch_size=500):
    ...
    with torch.no_grad():
        for i in range(math.ceil(len(text1) / batch_size)):
            features1.append(
                clip_model.encode_text(text1[batch_size * i : batch_size * (i + 1)])
            )
    ...
    with torch.no_grad():
        for i in range(math.ceil(len(text2) / batch_size)):
            features2.append(
                clip_model.encode_text(text2[batch_size * i : batch_size * (i + 1)])
            )
