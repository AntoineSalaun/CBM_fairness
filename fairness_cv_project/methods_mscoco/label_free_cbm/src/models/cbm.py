...
class CBM_model(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        model, _ = data_utils.get_target_model(backbone_name, device, "val")
...
def load_cbm(load_dir, device):
    ...
    proj_mean = torch.load(
        os.path.join(load_dir, "proj_mean.pt"), map_location="cpu"
    ).to(device)
    proj_std = torch.load(os.path.join(load_dir, "proj_std.pt"), map_location="cpu").to(
        device
    )
