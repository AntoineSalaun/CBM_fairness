...
from fairness_cv_project.methods_mscoco.label_free_cbm.src.plots import colors


def bar(
    contributions, feature_names, max_display=10, show=True, title=None, fontsize=13
):
    ...
    pl.barh(
        ...
        align="center",
        color=[
            colors.blue_rgb if values[feature_inds[j]] <= 0 else colors.red_rgb
            for j in range(len(y_pos))
        ],
        hatch=patterns[0],
        edgecolor=(1, 1, 1, 0.8),
        label=None,  # f"{cohort_labels[i]} [{cohort_sizes[i] if i < len(cohort_sizes) else None}]"
    )
...
def bar_percentage(
    ...
    if max_display < len(values):
        values[feature_order[max_display - 1]] = (
            np.sum(
                [values[feature_order[i]] for i in range(max_display - 1, len(values))],
                0,
            )
            + bias
        )
    ...
    pl.barh(
        y_pos,
        values[feature_inds] * conf / (np.sum(orig_values) + bias),
        bar_width,
        align="center",
        color=[
            colors.blue_rgb if values[feature_inds[j]] <= 0 else colors.red_rgb
    ...
    labels = [
        "{}{:.2f}%".format("+" if value >= 0 else "-", value)
        for value in values * conf / (np.sum(orig_values) + bias)
    ]
    ...
    for j in range(len(y_pos)):
        ...
        if values[ind] < 0:
            pl.text(
                values[ind] * conf / (np.sum(orig_values) + bias)
                - (5 / 72) * bbox_to_xscale,
                y_pos[j],
                labels[ind],  # format_value(values[ind], '%+0.02f'),
                horizontalalignment="right",
                verticalalignment="center",
                color=colors.blue_rgb,
                fontsize=fontsize,
