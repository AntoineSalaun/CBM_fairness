...
def main():
    ...
    path_dataset = (
        path_root / "data" / "mscoco" / "metadata" / "multi_label" / args.dataset
    )
    path_save = Path(f"saved_models/mscoco/multi_label/baseline/{args.save_name}")
    ...
    antn_files = {
        "default": "data/datasets/mscoco/metadata/multi_label/train.json",
    }

    images_path = "data/datasets/mscoco/images2014/"
    train_antn_file = antn_files[args.dataset]
    test_antn_file = "data/datasets/mscoco/metadata/multi_label/val.json"
    ...
    criterion = nn.BCEWithLogitsLoss(weight=object_weights, reduction='elementwise_mean')
