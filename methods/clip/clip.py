...
#    print('strating download function to ', download_target)
    ...
    if os.path.isfile(download_target):
        if hashlib.sha256(open(download_target, "rb").read()).hexdigest() == expected_sha256:
            print(filename, ' was already in ', download_target)
...
def load(path_saved_models: Path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None):
    """Load a CLIP model
    ...
    model_path = path_saved_models / name
