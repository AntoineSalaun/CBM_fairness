#!/bin/bash
#SBATCH -o slurm_logs/grid_search_resnet_200_verbs_full_baseline.sh.log-%j
    ...
    for momentum_value in "${MOMENTUM[@]}"; do
        for step_size_value in "${STEP_SIZE[@]}"; do
            for gamma_value in "${GAMMA[@]}"; do
                ...
                save_folder="200_verbs_full/grid_search/baseline/lr_${learning_rate_value}_momentum_${momentum_value}_step_${step_size_value}_gamma_${gamma_value}"
                
                path_model="saved_models/imSitu/200_verbs_full/grid_search/baseline/lr_${learning_rate_value}_momentum_${momentum_value}_step_${step_size_value}_gamma_${gamma_value}/model.pt"
                
                result_folder="results/imSitu/200_verbs_full/grid_search/baseline/lr_${learning_rate_value}_momentum_${momentum_value}_step_${step_size_value}_gamma_${gamma_value}"
